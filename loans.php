...
<div class="container-fluid">
					...
					<tbody>
						<?php
							...
							while($row = $qry->fetch_assoc()):
								...
								if($row['status'] == 1):
									$next = $conn->query("SELECT * FROM loan_schedules where loan_id = '".$row['id']."'  order by date_due asc limit 1 ")->fetch_assoc()['date_due'];
								endif;
								$sum_paid = 0;
 ...
 Visit website : www.mayurik.com -->  
						 	...
						 	<td>
						 		...
						 		<?php endif;  if($row['status'] == 0 ): ?>
					 				No next payment details,,, <br>loan not yet aproved<br>Still pending
									 <?php endif; if($row['status'] == 3 ):?>
										No next payment details,,, <br>Not allowed to get Loan<br>LOAN Denied
										<?php  endif; ?>
