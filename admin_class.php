...
Class Action {
	...
	function delete_borrower(){
		extract($_POST);
		$delete = $this->db->query("UPDATE  borrowers set status='deleted' where id = ".$id);
		if($delete)
			return 1;
	}
	function save_loan(){
			...
			if(isset($status)){
				...
				if($status == 1){
					...
					if($chk->num_rows > 0){
						...
						$this->db->query("UPDATE loan_schedules set loan_id = $id, date_due ='$date',payed='no' where id = $ls_id ");
					}else{
						$this->db->query("INSERT INTO loan_schedules set loan_id = $id, date_due ='$date',payed='no' ");
	...
	function delete_user(){
		extract($_POST);
		$delete = $this->db->query("UPDATE  users set status ='removed' where id = ".$id);
		if($delete)
			return 1;
	}
