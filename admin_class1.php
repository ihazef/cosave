	...
	function save_loan(){
		extract($_POST);
			...
			if ($status==0) {
				...
				$save = $this->db->query("UPDATE loan_list set status=0 where id=".$id);
			}
			if ($status==2) {
				$save = $this->db->query("UPDATE loan_list set status=2 where id=".$id);

			}
			if ($status==3) {
				$save = $this->db->query("UPDATE loan_list set status=3 where id=".$id);
