...
<div class="container-fluid">
 ...
 Visit website : www.mayurik.com -->  
					<tbody>
						<?php
						$i = 1;
							$qry = $conn->query("SELECT * FROM  borrowers  where borrowers.status='active' order by borrowers.id desc");
							while($row = $qry->fetch_assoc()):
								$b_id=$row['id'];
							if(	$loan=$conn->query("SELECT * from loan_list where borrower_id='$b_id'"))
								
								$loan=$loan->fetch_array();
								$lid=$loan['id'];
								$payed=$conn->query("SELECT * from payments list where loan_id='$lid'");
								$payed=$payed->fetch_array();
								
								$current=$loan['amount']+(($loan['amount']*5))/100-$payed['amount'];
						 ...
						 <tr>
						 	...
						 	<td>
						 		...
						 		<p>B_ID :<b><?php echo $row['tax_id'] ?></b></p>
						 		
						 	</td>
						 	<td class=""><?php echo $current > 1 ?  $current : $current; ?></td>
						 	<td class="">N/A</td>
...
<script>
	...
	$('.delete_borrower').click(function(){
		_conf("Are you sure you want to delete this borrower?","delete_borrower",[$(this).attr('data-id')])
	})
